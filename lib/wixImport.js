...
function parseWixBookingsCsv(csvText) {
  ...
  records.forEach((row, idx) => {
    ...
    const serviceCode = codeFromName(serviceName);
    const email = (row["Boeking Contact E-mail"] || "").trim().toLowerCase();
    ...
    if (serviceCode === "art_attack_room") {
      // "Bezette plaatsen" is bij Art Attack Room het aantal ROOMS (altijd
...
module.exports = { parseWixBookingsCsv, parseWixDate, parseDutchLongDate };
