...
// "marie.peeters@gmail.com" -> "m***@gmail.com". Genoeg om lokaal te zien of de
// juiste mail vertrekt, zonder een adreslijst in de logs op te bouwen.
function maskRecipient(to) {
  return String(to)
    .split(",")
    .map(addr => {
      const [user, domain] = addr.trim().split("@");
      if (!domain) return "***";
...
// De mock logde vroeger de VOLLEDIGE mailtekst. Voor de interne melding is dat
// klantnaam, e-mailadres en de vrije notitie (waarin volgens het schema
// allergieën terechtkomen), en voor een cadeaubon de code zelf. Op Vercel
// belandt dat in de function logs, die bewaard blijven en leesbaar zijn voor
// iedereen met projecttoegang — een AVG-verwerking zonder grondslag, veroorzaakt
// door één vergeten omgevingsvariabele (preview-deployments erven die standaard
// niet). Daarom: in productie hard falen, lokaal enkel ontvanger en onderwerp
// loggen, nooit de inhoud.
async function sendMail({ to, subject, text }) {
  if (!isConfigured()) {
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "E-mail is niet geconfigureerd (GMAIL_USER/GMAIL_APP_PASSWORD ontbreken). " +
        "Zet ze in de Vercel-omgevingsvariabelen en redeploy."
      );
    }
    console.log(`[email mock] naar ${maskRecipient(to)} — "${subject}" (${text.length} tekens, inhoud niet gelogd)`);
    return { mocked: true };
  }
