...
const { bestFitRoom, roomsForPartySize } = require("./rooms");
...
async function ensureSessionsMaterialized(pool, fromISO, toISO) {
  ...
  // Eerst uitrekenen WELKE sessies er in dit bereik zouden moeten staan, dan in
  // één keer opvragen wat er al is, en in één keer invoegen wat ontbreekt.
  //
  // Dit stond hier vroeger als een lus met per dag en per regel een aparte
  // SELECT en INSERT. Lokaal viel dat niet op, maar op Neon is elke query een
  // netwerkronde: een weekagenda kostte er zo 18 tot 33, een maandkalender 79
  // tot 133. Dat is waar de traagheid vandaan kwam.
  const gewenst = [];
  for (let dateISO = fromISO; dateISO < toISO; dateISO = addDaysISO(dateISO, 1)) {
    for (const rule of rules) {
      ...
      gewenst.push({
        ...
        ruleId: rule.rule_id,
        start,
        end: new Date(start.getTime() + rule.duration_minutes * 60000),
        capacity: rule.uses_room_assignment ? null : rule.default_capacity
      });
    }
  }
  if (gewenst.length === 0) return;

  const rangeStart = localDateTime(fromISO, "00:00");
  const rangeEnd = localDateTime(toISO, "00:00");
  const { rows: bestaand } = await pool.query(
    `SELECT service_id, start_datetime FROM sessions
      WHERE start_datetime >= $1 AND start_datetime < $2 AND service_id IS NOT NULL`,
    [rangeStart, rangeEnd]
  );
  const alAanwezig = new Set(bestaand.map(r => `${r.service_id}|${new Date(r.start_datetime).getTime()}`));

  const teMaken = gewenst.filter(g => !alAanwezig.has(`${g.serviceId}|${g.start.getTime()}`));
  if (teMaken.length === 0) return;

  // Eén INSERT met alle ontbrekende rijen. ON CONFLICT DO NOTHING vangt de race
  // op waarbij twee gelijktijdige verzoeken dezelfde sessie willen aanmaken —
  // dat vereist de unieke index uit migratie 008.
  const waarden = [];
  const params = [];
  teMaken.forEach((g, i) => {
    const b = i * 5;
    waarden.push(`('service', $${b + 1}, $${b + 2}, $${b + 3}, $${b + 4}, $${b + 5})`);
    params.push(g.serviceId, g.ruleId, g.start, g.end, g.capacity);
  });
  await pool.query(
...
/**
 * Bezette rooms voor MEERDERE sessies in één query, in plaats van één query per
 * sessie. Geeft een Map van sessie-id naar een lijst roomcodes.
 */
async function occupiedRoomCodesForSessions(pool, sessionIds, dbRooms) {
  const perSessie = new Map(sessionIds.map(id => [id, []]));
  if (sessionIds.length === 0) return perSessie;
  const { rows } = await pool.query(
    "SELECT session_id, room_id FROM room_bookings WHERE session_id = ANY($1::uuid[])",
    [sessionIds]
  );
  rows.forEach(rb => {
    const room = dbRooms.find(r => r.dbId === rb.room_id);
    if (room && perSessie.has(rb.session_id)) perSessie.get(rb.session_id).push(room.id);
  });
  return perSessie;
}

/**
 * Aantal geboekte personen per sessie, in één query. Voor diensten zonder
 * room-toewijzing (Fluid Art), waar de sessiecapaciteit telt.
 */
async function bookedPerSession(pool, sessionIds) {
  const perSessie = new Map(sessionIds.map(id => [id, 0]));
  if (sessionIds.length === 0) return perSessie;
  const { rows } = await pool.query(
    `SELECT session_id, COALESCE(SUM(party_size),0)::int AS total
       FROM bookings
      WHERE session_id = ANY($1::uuid[]) AND status NOT IN ('cancelled','rescheduled')
      GROUP BY session_id`,
    [sessionIds]
  );
  rows.forEach(r => perSessie.set(r.session_id, Number(r.total)));
  return perSessie;
}
...
/**
 * @param {boolean} [viaBackoffice] — true wanneer dit voor het
 *   boekingsscherm in /backend is. Een groep die niet in één room past mag
 *   daar wél geboekt worden (die neemt er dan twee in), dus wordt de
 *   boekbaarheid met dezelfde regel berekend als createManualBooking().
 */
async function getAvailability(serviceCode, dateISO, partySize, viaBackoffice = false) {
  const pool = await getPool();
  ...
  // Bezetting voor alle tijdsloten van de dag in één keer ophalen; vroeger was
  // dit een query per tijdslot.
  const sessieIds = sessions.map(s => s.id);
  const bezetting = service.uses_room_assignment
    ? await occupiedRoomCodesForSessions(pool, sessieIds, dbRooms)
    : null;
  const geboekt = service.uses_room_assignment ? null : await bookedPerSession(pool, sessieIds);
  ...
  for (const s of sessions) {
    ...
    if (service.uses_room_assignment) {
      const occ = bezetting.get(s.id) || [];
      if (!partySize) {
        bookable = dbRooms.some(r => !occ.includes(r.id));
      } else if (viaBackoffice) {
        bookable = roomsForPartySize(partySize, occ, dbRooms) !== null;
      } else {
        bookable = bestFitRoom(partySize, occ, dbRooms) !== null;
      }
      ...
      const remaining = s.capacity - (geboekt.get(s.id) || 0);
...
async function getMonthAvailability(serviceCode, year, month, partySize) {
  ...
  // Bezetting van de hele maand in één query in plaats van één per sessie.
  // Dit was de duurste plek van allemaal: een maand telt tot een vijftigtal
  // sessies, en dat waren evenveel netwerkrondes naar Neon.
  const sessieIds = sessions.map(s => s.id);
  const bezetting = service.uses_room_assignment
    ? await occupiedRoomCodesForSessions(pool, sessieIds, dbRooms)
    : null;
...
/**
 * @param {boolean} [viaBackoffice] — true bij een manuele boeking. De
 *   online-bovengrens (7 personen bij Action Painting) geldt dan niet: aan de
 *   balie of aan de telefoon komen er groepen van 15 of 20 binnen, en die
 *   moeten gewoon ingegeven kunnen worden (Robin, aug 2026). Boven de
 *   prijstrap wordt er per persoon doorgerekend, zie hieronder.
 */
async function priceForPartySize(pool, service, partySize, viaBackoffice = false) {
  ...
  if (!viaBackoffice && service.max_online_party_size && partySize > service.max_online_party_size) {
    throw new Error(
  ...
  if (service.uses_room_assignment) {
    ...
    if (rows[0]) return Number(rows[0].total_price);
    if (viaBackoffice) return await prijsBovenDeTrap(pool, service, partySize);
    throw new Error(`Geen prijs gekend voor groepsgrootte ${partySize} bij ${service.name}.`);
  }
  return Number(service.price) * partySize;
}

/**
 * Prijs voor een groep die boven de prijstrap valt (meer dan 7 bij Action
 * Painting). Er wordt doorgerekend aan de prijs per persoon van de HOOGSTE
 * trede: 7 personen kosten €364, dus €52 per persoon, en 15 personen wordt
 * €780.
 *
 * Bewust afgeleid uit de tabel en niet als vast getal in de code: past Robin
 * de prijstrap ooit aan, dan schuift dit vanzelf mee. Het bedrag is in de
 * backoffice trouwens ook gewoon te overschrijven (`amountOverride`) — voor
 * een grote groep wordt vaak apart onderhandeld.
 */
async function prijsBovenDeTrap(pool, service, partySize) {
  const { rows } = await pool.query(
    `SELECT party_size, total_price FROM service_party_pricing
      WHERE service_id = $1 ORDER BY party_size DESC LIMIT 1`,
    [service.id]
  );
  if (!rows[0]) throw new Error(`Geen prijstrap gekend voor ${service.name}.`);
  const perPersoon = Number(rows[0].total_price) / Number(rows[0].party_size);
  return Math.round(perPersoon * partySize * 100) / 100;
}

async function upsertCustomer(pool, { name, email, phone, birthDate, marketingOptIn }) {
  // E-mail is optioneel bij een manuele boeking (balie/telefoon). Zonder
  // e-mail valt er niets te herkennen: elke boeking krijgt dan een eigen
  // klantrij. Bewust NIET matchen op naam — twee verschillende "Jan Peeters"
  // zouden zo op elkaars gegevens en boekingsgeschiedenis terechtkomen.
  const schoonEmail = email && String(email).trim() ? String(email).trim() : null;
  const { rows: existing } = schoonEmail
    ? await pool.query("SELECT id FROM customers WHERE email = $1", [schoonEmail])
    : { rows: [] };
  if (existing[0]) {
    await pool.query(
  ...
  const { rows } = await pool.query(
    ...
    [name, schoonEmail, phone, birthDate, marketingOptIn !== false]
  );
  return rows[0].id;
}
...
/**
 * Een factuur kan enkel met een BTW-nummer (Robin, aug 2026): Billit maakt
 * anders een factuur op naam van een privépersoon, en dat is net wat een
 * klant die om een factuur vraagt niet nodig heeft.
 *
 * Bewust ook hier serverside en niet enkel in het formulier: een "required"
 * in de browser is een hulpje voor de klant, geen controle. Wie het verzoek
 * rechtstreeks opstuurt, omzeilt dat.
 */
function controleerFactuurgegevens(invoiceRequested, invoiceDetails) {
  if (!invoiceRequested) return;
  const btw = (invoiceDetails && invoiceDetails.vatNumber ? String(invoiceDetails.vatNumber) : "").trim();
  if (!btw) {
    throw new Error("Een BTW-nummer is verplicht wanneer je een factuur wenst.");
  }
}
...
async function createBooking(payload) {
  ...
  controleerFactuurgegevens(invoiceRequested, invoiceDetails);
  ...
  // De online bovengrens hier expliciet, VOOR de roomtoewijzing. Anders zou
  // een te grote groep terugkomen met "dit tijdslot is volzet" — wat niet
  // klopt: het tijdslot is vrij, de groep is gewoon te groot om zelf online
  // te boeken. Sinds aug 2026 kan het team zo'n groep wél manueel ingeven
  // (die neemt dan twee rooms in), dus deze grens geldt enkel voor de widget.
  ...
  // Online blijft het bewust één room per boeking: de widget verkoopt nooit
  // meer dan max_online_party_size (7) en dus nooit meer dan in één room past.
  // Meerdere rooms combineren kan enkel via een manuele boeking, zie
  // createManualBooking() hieronder.
  let roomDbId = null;
  const dbRooms = service.uses_room_assignment ? await getRooms(pool) : null;
  if (service.uses_room_assignment) {
...
/**
 * Notitie bij een boeking bewaren of aanpassen.
 *
 * De klant kan zelf een notitie meegeven bij het boeken (wensen, verjaardag,
 * allergieën), maar het team kon daar niets meer aan toevoegen — terwijl dat
 * precies de plek is waar "belt terug", "brengt eigen canvas mee" of "factuur
 * al bezorgd" thuishoort. Vandaar dit veld, bewerkbaar vanuit het
 * boekingsdetail (Robin, aug 2026).
 *
 * De notitie komt mee in de weekagenda, de PDF-export van een boeking en de
 * interne bevestigingsmail — dus geen plek voor gevoelige gegevens.
 */
async function updateBookingNote(bookingId, note) {
  const pool = await getPool();
  const tekst = String(note ?? "").slice(0, 2000); // harde grens, zie veiligheidsrapport
  const { rows } = await pool.query(
    `UPDATE bookings SET customer_note = $2 WHERE id = $1
       AND status NOT IN ('cancelled','rescheduled')
     RETURNING id, customer_note`,
    [bookingId, tekst]
  );
  if (!rows[0]) throw new Error("Boeking niet gevonden (of geannuleerd/verplaatst).");
  return { ok: true, note: rows[0].customer_note };
}
...
async function createManualBooking(payload) {
  const {
    ...
    reserveOnly, amountOverride
  } = payload;

  // Enkel de naam is verplicht. E-mail is optioneel sinds aug 2026: aan de
  // balie of aan de telefoon heeft niet elke klant er een bij de hand. Zonder
  // e-mail vertrekt er geen bevestigingsmail — bij een manuele boeking sturen
  // we die sowieso niet, het team heeft de klant net zelf gesproken.
  if (!customer || !customer.name) {
    throw new Error("De naam van de klant is verplicht.");
  }
  controleerFactuurgegevens(invoiceRequested, invoiceDetails);
  ...
  // Staat er nog geen sessie op dit uur, dan wordt die hier eenmalig
  // aangemaakt. Het team moet aan de telefoon een uur kunnen afspreken dat
  // niet in het vaste rooster staat (Robin, aug 2026) — precies zoals bij het
  // verplaatsen van een boeking. `recurrence_rule_id` blijft NULL: dit is een
  // losse sessie, het vaste uurrooster verandert er niet door.
  let session = sessRows[0];
  if (!session) {
    const einde = new Date(startDate.getTime() + (service.duration_minutes || SESSION_DURATION_MIN) * 60000);
    const { rows: nieuweSessie } = await pool.query(
      `INSERT INTO sessions (kind, service_id, recurrence_rule_id, start_datetime, end_datetime, capacity)
       VALUES ('service', $1, NULL, $2, $3, $4)
       ON CONFLICT DO NOTHING
       RETURNING *`,
      [service.id, startDate, einde, service.uses_room_assignment ? null : service.default_capacity]
    );
    if (nieuweSessie[0]) {
      session = nieuweSessie[0];
    } else {
      // ON CONFLICT: een gelijktijdig verzoek was net sneller — die sessie ophalen.
      const { rows: opnieuw } = await pool.query(
        "SELECT * FROM sessions WHERE service_id = $1 AND start_datetime = $2 AND kind = 'service'",
        [service.id, startDate]
      );
      session = opnieuw[0];
    }
    if (!session) throw new Error("Kon geen sessie aanmaken op dit tijdstip.");
  }

  // Rooms toewijzen. Anders dan online kan een manuele boeking groter zijn dan
  // één room aankan: past de groep niet in room A (10 plaatsen), dan komt er
  // automatisch een tweede room bij — VR, zoals afgesproken (Robin, aug 2026).
  // Beide rooms komen als aparte room_bookings-rij op dezelfde boeking, zodat
  // ze in de agenda allebei op naam staan en niemand er nog op kan boeken.
  let gekozenRooms = [];
  const dbRooms = service.uses_room_assignment ? await getRooms(pool) : null;
  if (service.uses_room_assignment) {
    const occ = await occupiedRoomCodes(pool, session.id, dbRooms);
    const keuze = roomsForPartySize(partySize, occ, dbRooms);
    if (!keuze) throw new Error("Dit tijdslot is helaas volzet voor deze groepsgrootte.");
    gekozenRooms = keuze.rooms;
  } else {
    const { rows: booked } = await pool.query(
      "SELECT COALESCE(SUM(party_size),0) AS total FROM bookings WHERE session_id = $1 AND status NOT IN ('cancelled','rescheduled')",
      [session.id]
  ...
  // viaBackoffice = true: de online bovengrens van 7 personen geldt hier niet,
  // en boven de prijstrap wordt er per persoon doorgerekend (€52, afgeleid uit
  // de hoogste trede). Heeft het team een eigen bedrag afgesproken, dan wint dat.
  let amount = await priceForPartySize(pool, service, partySize, true);
  if (amountOverride !== undefined && amountOverride !== null && amountOverride !== "") {
    const eigen = Number(amountOverride);
    if (!Number.isFinite(eigen) || eigen < 0) throw new Error("Geef een geldig bedrag op.");
    amount = Math.round(eigen * 100) / 100;
  }
  const subtotal = amount;
  ...
  for (const room of gekozenRooms) {
    await pool.query(
      "INSERT INTO room_bookings (session_id, room_id, booking_id, block_type) VALUES ($1,$2,$3,'booking')",
      [session.id, room.dbId, booking.id]
    );
  }
  ...
  return {
    booking: {
      ...
      roomCodes: gekozenRooms.map(r => r.id)
    }
  };
}
...
async function changePartySize(bookingId, { partySize, recalculatePrice = false } = {}) {
  ...
  // Bewust GEEN bovengrens meer: dit is een backoffice-actie. De online
  // limiet van 7 hoort bij de widget, niet bij wat het team aan de balie
  // ingeeft (Robin, aug 2026). Past de groep niet in één room, dan komt er
  // net als bij een nieuwe manuele boeking automatisch een tweede room bij.
  let newRooms = [];
  if (service.uses_room_assignment) {
    const dbRooms = await getRooms(pool);
    // De eigen room(s) bewust NIET als bezet meetellen: anders zou een boeking
    // die in de juiste room zit zichzelf blokkeren en "volzet" opleveren.
  ...
  if (recalculatePrice) {
    ...
    newSubtotal = await priceForPartySize(pool, service, size, true);
  ...
  let roomCode = null;
  if (service.uses_room_assignment && newRooms.length > 0) {
    await pool.query("DELETE FROM room_bookings WHERE booking_id = $1", [bookingId]);
    for (const room of newRooms) {
      await pool.query(
        "INSERT INTO room_bookings (session_id, room_id, booking_id, block_type) VALUES ($1,$2,$3,'booking')",
        [booking.session_id, room.dbId, bookingId]
      );
    }
    roomCode = newRooms.map(r => r.id).join(" + ");
  }
...
async function rescheduleBooking(bookingId, { dateISO, start }) {
  ...
  // Bestaat er nog geen sessie op dat moment, dan wordt er een eenmalige
  // aangemaakt (Robin, aug 2026). Een boeking verplaatsen naar een uur buiten
  // het vaste rooster is een normale zaak — een klant die vraagt om een half uur
  // later te mogen komen, bijvoorbeeld. Vroeger kreeg je hier "Dit tijdslot
  // bestaat niet" en moest je eerst via een ander scherm een extra sessie
  // aanmaken. recurrence_rule_id blijft NULL, dus het vaste rooster verandert niet.
  let newSession = sessRows[0];
  let sessieAangemaakt = false;
  if (!newSession) {
    const einde = new Date(startDate.getTime() + (service.duration_minutes || 90) * 60000);
    ...
    if (nieuw[0]) {
      newSession = nieuw[0];
      sessieAangemaakt = true;
    } else {
      const { rows: opnieuw } = await pool.query(
        "SELECT * FROM sessions WHERE service_id = $1 AND start_datetime = $2 AND kind = 'service'",
        [service.id, startDate]
      );
      newSession = opnieuw[0];
    }
    if (!newSession) throw new Error("Kon geen sessie aanmaken op dit tijdstip.");
  }
  if (newSession.id === oldBooking.session_id) {
    throw new Error("Dit is hetzelfde tijdslot als de huidige boeking.");
  }

  // Een grote groep (meer dan in één room past) neemt ook op het NIEUWE
  // tijdstip meerdere rooms in — anders zou een verplaatste boeking van 15
  // personen plots nog maar één room bezet houden.
  let nieuweRooms = [];
  if (service.uses_room_assignment) {
    const dbRooms = await getRooms(pool);
    const occ = await occupiedRoomCodes(pool, newSession.id, dbRooms);
    const keuze = roomsForPartySize(oldBooking.party_size, occ, dbRooms);
    if (!keuze) throw new Error("Dit tijdslot is helaas volzet voor deze groepsgrootte.");
    nieuweRooms = keuze.rooms;
  } else {
    const { rows: booked } = await pool.query(
      "SELECT COALESCE(SUM(party_size),0) AS total FROM bookings WHERE session_id = $1 AND status NOT IN ('cancelled','rescheduled')",
      [newSession.id]
  ...
  for (const room of nieuweRooms) {
    await pool.query(
      "INSERT INTO room_bookings (session_id, room_id, booking_id, block_type) VALUES ($1,$2,$3,'booking')",
      [newSession.id, room.dbId, newBooking.id]
    );
  }
...
async function importWixBooking({ serviceCode, dateISO, start, partySize, customer, note, paid }) {
  ...
  try {
    ...
    // Valt de boeking buiten het vaste uurrooster, dan wordt er alsnog een
    // eenmalige sessie aangemaakt (Robin, aug 2026). Deze boekingen bestaan
    // écht en zijn betaald; ze overslaan zou betekenen dat het tijdslot vrij
    // lijkt en een tweede klant erop kan boeken. Het uurrooster zelf blijft
    // ongemoeid — recurrence_rule_id is NULL, net als bij een handmatig
    // toegevoegde extra sessie. Ze worden apart geteld in het verslag, zodat
    // duidelijk is welke uren je nadien nog moet opkuisen.
    let session = sessRows[0];
    let sessieAangemaakt = false;
    if (!session) {
      const einde = new Date(startDate.getTime() + (service.duration_minutes || 90) * 60000);
      ...
      if (nieuw[0]) {
        ...
        // ON CONFLICT sloeg toe: een gelijktijdig verzoek was ons voor.
        const { rows: opnieuw } = await pool.query(
          "SELECT * FROM sessions WHERE service_id = $1 AND start_datetime = $2 AND kind = 'service'",
          [service.id, startDate]
        );
      ...
      if (!session) {
        return { status: "error", message: `Kon geen sessie aanmaken op ${dateISO} om ${start}.` };
      }
    }
    ...
    return sessieAangemaakt
      ? { status: "imported_new_session", bookingId,
          message: `Sessie bestond niet en is aangemaakt: ${dateISO} om ${start} (${service.name}).` }
      : { status: "imported", bookingId };
  } catch (err) {
...
async function maybeSendConfirmationEmail(pool, bookingId) {
  ...
  const booking = rows[0];
  if (!booking) return;
  // Een manuele boeking mag sinds aug 2026 zonder e-mailadres. Dan valt er
  // niets te versturen — geen fout, gewoon overslaan.
...
/**
 * Een eigen tijdsblok in de agenda (bv. "Kamp voorbereiden", "Zaal poetsen").
 *
 * Bewuste keuze (Robin, aug 2026): een tijdsblok neemt GEEN rooms in en
 * blokkeert GEEN online boekingen — het is een gekleurde melding voor het
 * team, meer niet. Wie wél wil dat er niet meer op geboekt kan worden,
 * gebruikt "Room(s) sluiten" (closeRoom hierboven); dat blijft een aparte,
 * expliciete handeling zodat je nooit per ongeluk je eigen agenda dichtzet.
 *
 * Verschil met een persoonlijke afspraak: die is privé (de gast-rol ziet
 * enkel "Privé"), een tijdsblok toont gewoon zijn titel.
 */
async function addTimeBlock({ title, dateISO, start, end }) {
  if (!title || !dateISO || !start || !end) {
    throw new Error("Titel, datum, start- en eindtijd zijn verplicht voor een tijdsblok.");
  }
  if (end <= start) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO sessions (kind, title, start_datetime, end_datetime, visibility)
     VALUES ('block', $1, $2, $3, 'standard') RETURNING id`,
    [String(title).slice(0, 200), startDate, endDate]
  );
  return { id: rows[0].id, kind: "block", title, dateISO, start, end, status: "scheduled", visibility: "standard" };
}

/**
 * Verwijdert een tijdsblok. Enkel kind = 'block' — een workshopsessie of een
 * persoonlijke afspraak mag hier nooit langs, ook niet als iemand het id
 * ervan meestuurt.
 */
async function deleteTimeBlock(id) {
  const pool = await getPool();
  const { rowCount } = await pool.query(
    "DELETE FROM sessions WHERE id = $1 AND kind = 'block'",
    [id]
  );
  if (rowCount === 0) throw new Error("Tijdsblok niet gevonden.");
  return { ok: true };
}

/**
 * Past de titel en/of de uren van een bestaand tijdsblok aan.
 */
async function updateTimeBlock(id, { title, dateISO, start, end }) {
  if (!title || !dateISO || !start || !end) {
    throw new Error("Titel, datum, start- en eindtijd zijn verplicht voor een tijdsblok.");
  }
  if (end <= start) {
  ...
  const pool = await getPool();
  const { rowCount } = await pool.query(
    `UPDATE sessions SET title = $2, start_datetime = $3, end_datetime = $4
     WHERE id = $1 AND kind = 'block'`,
    [id, String(title).slice(0, 200), localDateTime(dateISO, start), localDateTime(dateISO, end)]
  );
  if (rowCount === 0) throw new Error("Tijdsblok niet gevonden.");
  return { id, kind: "block", title, dateISO, start, end };
}
...
/**
 * Heft een room-sluiting weer op.
 *
 * Tegenhanger van closeRoom(): die kon je wel dichtzetten maar niet meer
 * openen — dat kon tot nu toe alleen rechtstreeks in de database (Robin,
 * aug 2026).
 *
 * Raakt uitsluitend rijen met block_type = 'closed'. Een room die bezet is door
 * een echte boeking (block_type = 'booking') blijft dus altijd staan; die geef
 * je vrij door de boeking te annuleren of te verplaatsen, niet hierlangs.
 *
 * roomId weglaten of allRooms meegeven heropent alle gesloten rooms van dat
 * tijdslot (of van de hele dag met allDay).
 */
async function reopenRoom({ dateISO, start, roomId, allRooms, allDay }) {
  const pool = await getPool();
  if (!dateISO) throw new Error("Datum is verplicht.");
  if (!allDay && !start) throw new Error("Geef een tijdstip op, of kies de hele dag.");
  if (!allRooms && !roomId) throw new Error("Geef een room op, of kies alle rooms.");
  ...
  if (!allDay && start) {
    params.push(localDateTime(dateISO, start));
    timeClause = " AND s.start_datetime = $3";
  }

  let roomClause = "";
  if (!allRooms) {
    params.push(roomId);
    roomClause = ` AND rb.room_id = (SELECT id FROM rooms WHERE code = $${params.length})`;
  }

  const { rowCount } = await pool.query(
    `DELETE FROM room_bookings rb
      USING sessions s
      WHERE rb.session_id = s.id
        AND rb.block_type = 'closed'
        AND s.start_datetime >= $1 AND s.start_datetime <= $2${timeClause}${roomClause}`,
    params
  );

  return { ok: true, heropend: rowCount };
}
...
async function getWeekSessions(mondayISO) {
  ...
  const { rows } = await pool.query(
    `SELECT s.id AS session_id, s.kind, s.title, s.start_datetime, s.end_datetime,
            ...
            b.payment_status, b.refunded_amount, b.booked_via, b.invoice_requested,
            b.billit_invoice_id, c.full_name AS customer_name,
  ...
  const events = rows.map(r => {
    ...
    if (r.kind === "block") {
      // Een eigen tijdsblok: enkel een titel en uren, geen dienst, geen room,
      // geen klant. Zichtbaar met titel (in tegenstelling tot 'personal').
      return {
        kind: "block",
    ...
    if (r.booking_id) {
      return {
        ...
        // Voor het detailvenster: is er een factuur gevraagd, en is die in
        // Billit ook echt aangemaakt? Zonder dit kon je in de agenda niet zien
        // of de facturatie gewerkt heeft.
        invoiceRequested: r.invoice_requested,
        billitInvoiceId: r.billit_invoice_id || null,
...
/**
 * Alle sessies + boekingen in een periode, met álle velden die in een export
 * horen — dus óók klant-e-mail, telefoon en geboortedatum, en het einduur.
 *
 * Waarom niet gewoon getWeekSessions() hergebruiken: die voedt de week-grid op
 * het scherm en geeft bewust géén e-mail/telefoon/geboortedatum terug (minder
 * persoonsgegevens door de browser = beter). Een export van 8 weken zou er
 * bovendien 8 keer op moeten draaien; dit doet het bereik in 2 queries, hoeveel
 * weken je ook exporteert.
 *
 * @param {string} fromISO  eerste dag (inclusief), "YYYY-MM-DD"
 * @param {string} toISO    laatste dag (INclusief), "YYYY-MM-DD"
 */
async function getAgendaExportRows(fromISO, toISO) {
  const pool = await getPool();
  const exclusiefISO = addDaysISO(toISO, 1);
  await ensureSessionsMaterialized(pool, fromISO, exclusiefISO);

  const rangeStart = localDateTime(fromISO, "00:00");
  const rangeEnd = localDateTime(exclusiefISO, "00:00");

  const { rows } = await pool.query(
    `SELECT s.id AS session_id, s.kind, s.title, s.start_datetime, s.end_datetime,
            s.capacity, s.status, s.visibility, sv.name AS service_name,
            sv.uses_room_assignment,
            b.id AS booking_id, b.party_size, b.customer_note, b.amount_due,
            b.status AS booking_status, b.payment_status, b.refunded_amount,
            b.booked_via, b.created_at AS booked_at,
            c.full_name AS customer_name, c.email AS customer_email,
            c.phone AS customer_phone, c.birth_date AS customer_birth_date,
            r.code AS room_code
     FROM sessions s
     LEFT JOIN services sv ON sv.id = s.service_id
     LEFT JOIN bookings b ON b.session_id = s.id AND b.status NOT IN ('cancelled','rescheduled')
  ...
  const sessies = rows.map(r => ({
    kind: r.kind === "personal" || r.kind === "block" ? r.kind : "service",
    title: r.title,
    service: r.service_name ? codeFromName(r.service_name) : null,
    ...
    customerBirthDate: r.customer_birth_date ? pgDateToISO(r.customer_birth_date) : null,
    partySize: r.party_size,
    note: r.customer_note || "",
    amount: r.amount_due !== null && r.amount_due !== undefined ? Number(r.amount_due) : null,
    refundedAmount: Number(r.refunded_amount || 0),
    paymentStatus: r.payment_status,
  ...
  const gesloten = closedRows.map(r => ({
    dateISO: toISODate(r.start_datetime),
    start: hhmm(r.start_datetime),
    roomCode: r.room_code,
    reason: r.reason || "Gesloten"
