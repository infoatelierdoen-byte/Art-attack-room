...
function ruleAppliesOn(rule, dateISO) {
  ...
  // anchor_date delen (bv. alle Art Attack Room-dagen ankeren op dezelfde
...
async function maybeRedeemGiftCard(pool, bookingId) {
  ...
  try {
    ...
    const newRemaining = Math.max(0, Math.round((Number(card.remaining_amount) - amount) * 100) / 100);
    const newStatus = newRemaining <= 0 ? "depleted" : card.status;

    await pool.query("UPDATE gift_cards SET remaining_amount = $1, status = $2 WHERE id = $3", [
      newRemaining, newStatus, card.id
    ]);
    await pool.query(
...
async function createGiftCardPurchase({ amount, purchaser }) {
  ...
  if (amt > 500) throw new Error("Maximumbedrag voor een cadeaubon online is €500. Neem contact op voor een hoger bedrag.");
...
async function fulfillGiftCardPurchase({ amount, purchaserName, purchaserEmail, recipientNote, molliePaymentId }) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO gift_cards (
     ...
     ) VALUES ($1,$2,$2,'active',$3,$4,$5,'purchased',$6,$7) RETURNING *`,
  ...
  try {
    ...
    console.error(`Cadeaubon-mail mislukt voor ${card.code}:`, err.message);
  }
...
async function createBooking(payload) {
  ...
  // Cadeaubon: enkel valideren en het te verzilveren bedrag onthouden op de
  // boeking zelf — het echte afschrijven van het saldo gebeurt bewust pas
  // zodra de betaling (van het eventuele restbedrag) bevestigd is, zie
  // maybeRedeemGiftCard() hieronder. Dekt de cadeaubon het volledige
  // bedrag, dan is er geen Mollie-betaling nodig en wordt meteen afgerekend.
  let giftCard = null;
  ...
  const { rows: bookingRows } = await pool.query(
    `INSERT INTO bookings (
       ...
       gift_card_id, gift_card_amount, payment_status
...
async function cancelBooking(bookingId, { refundAmount = 0, reason } = {}) {
  ...
  // Raakt bewust NIET aan een eventuele gebruikte cadeaubon — die
  // terugstorting blijft voorlopig manueel werk.
  const newPaymentStatus = refund >= amountDue && amountDue > 0 ? "refunded" : booking.payment_status;
  await pool.query(
    `UPDATE bookings
     SET status = 'cancelled', payment_status = $2, refunded_amount = $3,
         refund_reason = $4, refunded_at = CASE WHEN $3 > 0 THEN now() ELSE refunded_at END
     WHERE id = $1`,
  ...
  return { ok: true, refundedAmount: refund };
}
...
async function addExtraSession({ serviceCode, dateISO, start, capacity }) {
  ...
  // Fluid Art) — bij Art Attack Room bepaalt de roomtoewijzing zelf de
...
async function getWeekSessions(mondayISO) {
  ...
  const { rows } = await pool.query(
    `SELECT s.id AS session_id, s.kind, s.title, s.start_datetime, s.end_datetime,
            ...
            b.payment_status, b.booked_via, c.full_name AS customer_name,
